...
from app.ranking import rank_and_select, get_item_id
from app.llm import generate_daily_report, generate_final_summary
from app.push import PushPlusSender, EmailSender, WeComSender
...
def run_push_task(window_days: int = None, top_k: int = None):
    ...
    try:
        ...
        from app.llm.quick_check import quick_relevance_check
        from app.models import ScoreReason
        ...
        for paper_idx, scored_paper in enumerate(all_scored_papers, 1):
            ...
            try:
                ...
                from app.llm.generator import generate_single_paper_report
                ...
                # 即使失败，也生成一个简单的降级报告
                from app.llm.generator import _generate_fallback_report
...
def save_report_to_file(content: str, papers_count: int, source_results: List, run_id: str):
    ...
    filename = f"reports/生化领域突破汇总_{today}.txt"
